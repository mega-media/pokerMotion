...
import { clone, max, min } from 'ramda';
...
export const setCardParams = (card) => {
  card.anchor.set(1, 0);
  card.getChildAt(0).anchor.set(1, 0);
  _cardInit(card);
};
...
const _moving = (card, mask) => (x, y) => {
  ...
  if (leftPosX <= ORIGIN_POINTS[0][0]) {
    ...
    maskPoints[3] = [ORIGIN_POINTS[0][0], -1 * unSlope * ELE_WIDTH + rightPosY];
    maskPoints[4] = [0, ORIGIN_POINTS[3][1]];
    mask.refresh(maskPoints);

    const angle = angleBetweenPoints(maskPosX, ORIGIN_POINTS[2][0], maskPosY, rightPosY);
    card.rotation = Math.PI * 2 * (0.5 + ( angle / 360));

  } else if (rightPosY <= ORIGIN_POINTS[0][1]) {
    /* 右四邊形 */
    maskPoints[1] = [ORIGIN_POINTS[1][0], 0];
    maskPoints[2] = [-1 * ELE_HEIGHT / unSlope + leftPosX, ORIGIN_POINTS[0][1]];
    maskPoints[3] = [leftPosX, ORIGIN_POINTS[2][1]];
    maskPoints[4] = [0, ORIGIN_POINTS[3][1]];
    mask.refresh(maskPoints);

    const angle = angleBetweenPoints(maskPosX, leftPosX, maskPosY, ORIGIN_POINTS[2][1]);
    card.rotation = Math.PI * 2 * (0.25 + ( angle / 360));
